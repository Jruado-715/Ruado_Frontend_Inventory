// ─── ProductsPage.jsx ─────────────────────────────────────────────────────────
// Full CRUD with search, filter, sort — connected to MySQL via Laravel API.
// Products update in real time via WebSocket (Laravel Echo + Reverb).
...
const inputStyle = { width:"100%", background:"#222", border:"1px solid #2a2a2a", borderRadius:"3px", padding:"8px 10px", fontFamily:"'Barlow',sans-serif", fontSize:"0.88rem", color:"#fff", outline:"none" };
const labelStyle = { display:"block", fontSize:"0.65rem", fontWeight:700, letterSpacing:"0.12em", textTransform:"uppercase", color:"#757575", marginBottom:"4px" };
...
const ProductForm = ({ initial = {}, onSave, onCancel }) => {
  ...
  return (
    <div>
      {err && <div style={{ background:"rgba(229,9,20,0.1)", border:"1px solid rgba(229,9,20,0.3)", borderRadius:"3px", padding:"6px 10px", fontSize:"0.8rem", color:"#ff6b6b", marginBottom:"10px" }}>{err}</div>}

      {/* Icon picker — compact single row */}
      <div style={{ marginBottom:"10px" }}>
        <label style={labelStyle}>Icon</label>
        <div style={{ display:"flex", gap:"4px", flexWrap:"wrap" }}>
          {EMOJIS.map((e, i) => (
            <button key={e} title={EMOJI_NAMES[i]} onClick={() => set("img", e)}
              style={{ fontSize:"1.1rem", background: form.img===e ? "rgba(229,9,20,0.2)" : "#222", border:`1px solid ${form.img===e ? "#E50914" : "#2a2a2a"}`, borderRadius:"4px", padding:"3px 6px", cursor:"pointer", lineHeight:1 }}>
              {e}
            </button>
          ))}
        </div>
      ...
      <div style={{ display:"flex", gap:"8px", justifyContent:"flex-end" }}>
        <button onClick={onCancel} style={{ background:"none", border:`1px solid ${colors.border}`, color:colors.muted, borderRadius:"3px", padding:"8px 18px", fontFamily:fonts.body, fontSize:"0.85rem", cursor:"pointer" }}>Cancel</button>
        <button onClick={submit} className="btn-red" style={{ background:colors.red, border:"none", color:"#fff", borderRadius:"3px", padding:"8px 20px", fontFamily:fonts.display, fontSize:"0.9rem", letterSpacing:"0.1em", cursor:"pointer" }}>
...
const ProductsPage = () => {
  ...
  const applyFilters = (newSearch, newCat, newSort) => {
    ...
    fetchProducts({ search: newSearch, category: newCat, sort_by, sort_dir });
  };
    ...
    <div style={{ animation:"fadeSlide 0.35s ease both" }}>
      ...
      <div style={{ display:"flex", gap:"10px", marginBottom:"20px", flexWrap:"wrap", alignItems:"center" }}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", gap:"6px" }}>
          {["grid","table"].map(v=>(
            <button key={v} onClick={()=>setView(v)} title={v === "grid" ? "Grid view" : "Table view"}
      ...
      {modal==="add"        && <Modal title="Add New Product" onClose={()=>setModal(null)} width="560px"><ProductForm onSave={handleAdd} onCancel={()=>setModal(null)}/></Modal>}
      {modal?.type==="edit" && <Modal title="Edit Product"    onClose={()=>setModal(null)} width="560px"><ProductForm initial={modal.product} onSave={handleEdit} onCancel={()=>setModal(null)}/></Modal>}
